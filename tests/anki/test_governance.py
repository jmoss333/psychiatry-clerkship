...
def test_valid_withdrawal_requires_exact_current_neutral_render_hash():
    ...
    assert reconcile_quarantines((finding,), {"accepted": [valid]}).accepted == (
        finding,
    )

    valid["approvedWithdrawalSha256"] = "f" * 64
    drift = reconcile_quarantines((finding,), {"accepted": [valid]})
...
def duplicate_findings(
    ...
    *,
    ...
    inputs = replace_inputs(bundle.inputs, cards=(first, second, *extras))
...
def test_configured_qbank_hold_is_detected_with_ownerless_proposal_fields(
    ...
    inputs = replace_inputs(
        bundle.inputs,
        qbank_items=(held_item,),
        release_config=config,
        first_seen_commit="27e2ef26245145bedfb997dfc424e7c91e72bb40",
    )
