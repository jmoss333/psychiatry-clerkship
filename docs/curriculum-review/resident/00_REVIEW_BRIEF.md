...
**Generated:** 2026-09-05 from build `3e6ca0e` · exporter: `13_Faculty_Resources/_automation/export_curriculum_review.py`

## Who this is for

Psychiatry residents on an adult inpatient rotation. The resident site is derived from the MS3 build, then overridden: resident welcome/rotation pages, advanced psychopharmacology, systems & med-legal, supervision & teaching, the 200-paper canon, a C-L numbers reference, resident Case-of-the-Week variants, and three resident-only rehearsal tools (rp-*). Expectations are supervision-level: independent decisions, teaching others, and med-legal accountability.
...
| Words of narrative curriculum | 121,255 |
...
`RESIDENT_CURRICULUM_COMPLETE.md` is every file above concatenated in order — the single running document. The split files exist so a reviewer can work one reviewable pass at a time.
...
| **S5 — level mismatch** | Correct but pitched wrong for this audience (RESIDENT) — too advanced, too thin, or a responsibility the learner does not actually hold. |
