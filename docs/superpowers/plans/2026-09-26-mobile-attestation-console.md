...
Two carried edits to Task 5's code ride in this task (same file):
- `resetAcks(nextStatus)`: when the new preview status is `'ready'`, keep `reviewedRevision` (the saved-draft receipt) alongside `retryAttempted` — the desktop's `clearReviewAcknowledgements({ preserveQuestionReceipts })`; every other flag resets. Call it as `resetAcks(event.data.status)` in `handlePreviewStatus`.
- In both `signContent` and `signQuestion`, after the write succeeds advance only when the reviewer is still on that item (`state.screen === 'item' && state.selectedKey === item.key`); otherwise refresh in place (`render()` for the queue, `refreshItem()` for another item) and keep `state.receipt` — a sign that completes after the reviewer pressed Queue or Lock must never pull them into the next item or repaint the key gate.
