...
| 1 | **WP-06R-a** | Retire the C-SSRS verbatim reproduction from `cssrs.html` | AGENT + AUTHOR-GATED | **stage 1 done** | — | — | stage 1 (2026-08-27): stems, branching, and triage engine removed — `cssrs.html` is a rights stub on the `bfcrs.html` #400 pattern, guarded by `tests/cssrs-retirement.test.mjs`; crisis block and tool counts unchanged. Stage 2 — the authored administration-teaching copy (SPEC draft) — remains AUTHOR-GATED on attestation + LOCAL_POLICY fill |
| 1 | **WP-06R-b** | Safety-plan rehearsal tool that reproduces no Stanley–Brown content | AUTHOR-GATED | **shell built** | — | — | content. Shell built 2026-09-05 in `_prototypes/safety-planning/` under the spec's own authorisation ("Claude Code may build the shell against this structure but must not treat any string as final"). Six-step generate-then-reveal loop; the three spec §1 invariants are enforced in code (no persisted free text, **no export of any kind**, `cw_*` keys) and re-checkable by grep per the prototype README. Deliberately **unregistered** from `site_manifest.json` and nav, so unattested clinical strings on a suicide-safety surface ship to neither site. Effect sizes omitted pending `sourceSpan`s. Still author-gated on the spec §7 checklist — every box open, MS3-vs-resident scope included |
