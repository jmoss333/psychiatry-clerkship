...
6. **The SP state machine is duplicated and parity-tested.** *(Amended 2026-08-20 — see amendment A2 in the STATUS tracker for the full mirror-surface table; enumerate it before editing.)* `deriveState` exists server-side at `sp-proxy/netlify/functions/sp.mjs:284`. **Client-side there is no function of that name** — the mirrored logic lives in `MockProvider`'s prototype methods in `_prototypes/sp-interview/sp-interview.html` (state literal L234, raise loop L264, penalty L273-275, gates L286-298, `computeCoverage` L368), exposed for testing as `window.__SP_TEST__`. `_prototypes/sp-interview/tests/parity.test.mjs` `eval`s the client `<script>` block and compares rapport / coverage / unlocked gates against the server's `_internals`. **Any change to one must be made to the other in the same commit** or parity fails. A one-sided "fix" that keeps parity green because both sides are wrong is a known failure mode in this repo (see the `onlyFirstTime` bug). **And parity does not cover the whole mirror:** the client's technique derivation (L394-397) and debrief strengths (L408-409) have no server counterpart and no parity coverage, so a WP-30 change missed there stays green. Grep for the quoted code string to locate every site — never trust an `L###` in this document (amendment A1).
