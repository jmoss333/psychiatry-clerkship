...
> **Scope note.** This case is deliberately oriented toward **recognition, structured assessment, risk *formulation*, escalation, and collaborative safety planning**. It contains no instructional method detail and requires none.
>
> **That is a rule about what this document prints, not about what you ask your patient.** A competent assessment requires direct questions about the **type of ideation (passive vs. active), intent, whether a plan exists and how specific and feasible it is, preparatory behaviour, prior attempts, and access to lethal means** — and a plan's feasibility cannot be judged without knowing what the patient can actually reach. Residents are the ones who model this for students, so make the distinction explicit when you teach it: a learner who comes away thinking the sensitivity of the topic is a reason to ask less has learned the wrong lesson, and will write a note that cannot support a disposition.
