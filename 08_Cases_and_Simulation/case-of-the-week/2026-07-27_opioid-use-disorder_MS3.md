...
| **Buprenorphine** (± naloxone) | **Partial** mu agonist, high receptor affinity | Office-based prescribing; ceiling effect on respiratory depression makes it comparatively safe. In **standard initiation** — the route you will see most often and the one this case uses — it is **started once the patient is in objective withdrawal**: given earlier, its high receptor affinity displaces the full agonist and precipitates withdrawal. With illicitly manufactured fentanyl that rule is necessary but no longer sufficient: fentanyl accumulates in tissue, so precipitated withdrawal still occurs in a minority of inductions done at an adequate withdrawal score, which is why induction here is planned with addiction medicine rather than run off a threshold [3]. Know that standard initiation is **not the only route**: in protocol-directed **low-dose initiation** the patient *continues* the full agonist while small, gradually escalating buprenorphine doses are added, so no withdrawal window is required — used when someone cannot tolerate withdrawal or has failed a standard start. It needs days, a written protocol, and a cooperative team; the evidence behind it is observational, and it is a specialist decision, not a bedside improvisation. (The resident version of this case works through that evidence.) |
...
- **Common MS3 errors to correct:**
  ...
  4. Believing buprenorphine can simply be started at any time — not recognizing why **standard** initiation waits for objective withdrawal. (The opposite over-correction is worth naming too: "buprenorphine *always* requires withdrawal first" is also wrong, because protocol-directed low-dose initiation exists.)
