...
- `.claude/settings.json` + `.claude/hooks/` — session hooks that enforce the rules below at edit
  ...
  the matching **pre-commit** gate for hand edits. Project hooks do not run inside subagents —
  their tool allowlists are their enforcement. `tests/hooks.test.mjs` drives every hook.
