...
def _rights(f,t=None,hidden=None):
    """A reference page ABOUT an instrument this library does not reproduce.

    Same .html surface as _tool, deliberately NOT the same kind: the page teaches
    administration and points at the official form, so presenting it as an interactive
    tool sends a learner reaching for a scorer mid-shift to a removal notice
    (Fresh Eyes Audit A3). fd_data.js's 'rights' kind overrides its /\\.html$/ heuristic,
    which is what keeps these out of Quick Tools and the Interactive-tools column."""
    return dict({"t":t or _tool_titles.get(f,f),"f":f,"k":"rights"},**({"hidden":True} if (hidden if hidden is not None else f in HIDDEN_TOOLS) else {}))
...
nav=[
 ...
 {"section":"Assess Safety and Acuity","pinned":True,"items":[_md("Suicide Risk & Safety","pg_suicide.md"),_md("Suicide Risk & Safety Planning","suicide.md"),_rights("cssrs.html","Columbia C-SSRS — Official Form & Training"),_md("Violence Risk","violence.md"),_tool("violence.html","Violence Risk (FRST)"),_md("Agitation & Restraint","agitation.md"),_md("Catatonia","catatonia.md"),_rights("bfcrs.html","Bush-Francis Catatonia Scale (BFCRS) — Official Form & Training"),_md("Hyperthermia & Toxidromes","toxidromes.md"),_md("Delirium","delirium.md"),_tool("withdrawal.html","Withdrawal: CIWA-Ar/COWS"),_tool("capacity.html","Decisional Capacity"),_md("Consult Questions: Capacity, Delirium, Catatonia, Withdrawal","exp_consult.md"),_md("Ethics & the Law: Confidentiality, Tarasoff, Reporting","ethics_legal.md")]},
