...
**B2. Cap auto-recharge.** **Status: RESOLVED DIFFERENTLY 2026-09-25** — Netlify's auto-recharge setting has no ceiling, only Enabled/Disabled, and Disabled means the learner sites can stop serving when credits hit zero (at the measured burn, ~9 days before the cycle resets — mid-rotation). Auto-recharge stays ON; the cap is a spend tripwire in `release_train.py` (`DEPLOY_BUDGET_24H = 12` billable production deploys across all five sites per trailing 24 h): a scheduled promotion that would exceed it holds and the run goes red; publish-now is never held. Not covered: AI-inference credits (Agent Runners / AI Gateway — 477.6 credits, 4.7M Anthropic tokens by 2026-09-25) and bandwidth; watch those on the billing page. Original text follows. It is on, it has fired 7 times in 25 days, and nothing told you. Billing → Credit balance → *Configure auto recharge*: set a deliberate monthly ceiling (2 packs/month is a sensible tripwire). With a cap, a runaway loop shows up as a stalled deploy you notice in minutes rather than a $200 invoice you notice in a month.
