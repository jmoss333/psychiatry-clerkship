...
def normalize_tool(
    ...
    *,
    ...
    reviewStatus, attestationStatus, safetySeverity, and reviewCategory come
    from the canonical ledger record alone. The source marker's own status/
    attestation/category/severity claims are never authoritative here — they
    remain inputs to the attestation-consistency validator, not this envelope.
    """
    ...
    envelope = {
        ...
        "reviewStatus": "reviewed" if ledger_status == "reviewed" else "needs-review",
    ...
    *,
    ...
    for source_relative, built_slug in _tool_entries(root, site):
        ...
        if ledger_entry is None:
            raise GovernanceError(f"{built_slug}: missing canonical ledger record")
        items.append(
            normalize_tool(
                source,
                relative_path,
