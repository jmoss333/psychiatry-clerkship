...
- **Netlify LFS env vars set** on both sites: `GIT_LFS_ENABLED=true` and `GIT_LFS_FETCH_INCLUDE=*.m4a,*.mp4` for builds. *(Superseded 2026-09-02, then measured 2026-09-14: both were **removed** from both sites on 2026-09-14, and removing them changed nothing. Netlify's checkout materialises LFS objects whether or not they are set; a cache-reusing build downloads ~0 MB, and the real cost is ~455 MB per **fresh clone**, i.e. per "Clear cache and deploy". See `site_build/NETLIFY_LFS_RUNBOOK.md`.)*
...
**Quota note:** the current tree has roughly 433 MB of LFS-tracked media, which fits GitHub LFS storage, but LFS **bandwidth** is metered per account (10 GB/mo on the current plan, reset on the 1st). **2026-08-30 incident:** it bit. The month's quota went on roughly 11 full re-fetches of the ~433 MB tree (previews and CI never fetch) — *corrected 2026-09-14: those were fresh clones (cache clears), not ordinary production builds, which reuse Netlify's cached repo and download nothing;* GitHub's 90% and 100% notices arrived 40 minutes apart and every production deploy of both sites failed the LFS gate until the 2026-09-01 reset. Runbook: `site_build/NETLIFY_LFS_RUNBOOK.md`, "Incident pattern 2".

### 6a. LFS bandwidth — cached pull (IMPLEMENTED 2026-09-02; INERT, measured 2026-09-14) and the remaining escape hatches
...
## 7. Build-ignore — retired 2026-09-03, RE-SCOPED 2026-09-10 (satellites only)
From 2026-07-02 to 2026-09-03 `netlify.toml` registered `site_build/netlify-ignore.sh` as a build-ignore hook: it cancelled the build when every changed file was a Markdown doc under `_automation/` or anything under `_automation/surveillance/`, saving a ~40 s build per site per doc-only push. It was retired on 2026-09-03 in favour of `ignore = "/bin/false"` (always build) in every `netlify.toml` — exit code 1 = "content changed, build" — which also overrode Netlify's *default* rule for the sites with a base directory. **On 2026-09-10 that was narrowed:** the root file still says `/bin/false`, because it governs the two learner sites, which are built from the whole repository; the four satellite files (`sp-proxy/`, `faculty-console/`, `metrics/`, `Outreach/alex-tour/`) now run `site_build/netlify_ignore_scoped.sh <their own directory>` and skip a **production** build whose diff does not touch that directory. Deploy previews and branch deploys always build.
...
**Reading a deploy list after this change.** A "Canceled" entry on a **satellite** site (`sp-interview-proxy`, `clerkship-faculty-attest`, `psychiatry-workforce-tour`) is now the normal, intended outcome of a merge that did not touch its directory. A "Canceled" entry on `une-ms3-psychiatry` or `mmc-psychiatry-residents-sanford` is a defect: someone scoped a site that must not be scoped, or removed `ignore = "/bin/false"` from the root toml. "Failed" still means a real failure — read the log. `tests/maintenance/test_netlify_ignore_scoped.py` pins both halves of that rule.
