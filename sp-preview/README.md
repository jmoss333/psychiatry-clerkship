...
### Family entry and speaker orientation

The hosted family visit begins with the clinician. Before Start, the learner reviews a short visible orientation and explicitly acknowledges it. No microphone or encounter request starts from selecting a case or reviewing instructions. The acknowledgment is an interface safeguard, not a claim that the software can prove reading or comprehension. It resets for a new family encounter.

Family Start authenticates and admits the encounter with the existing one-unit Start reservation, but makes no actor or speech request. Its `ready` then `complete` stream carries an empty clinician-first receipt; Morgan's older scripted opening is not spoken, shown, or inserted into history. The first actual question remains turn one, and the family still gets ten learner turns. Non-family openings are unchanged.

Begin a question with “Morgan” or “Maya” to address that person. Without a name, the selected person answers (Morgan initially). One person answers per turn and both remain in the shared meeting. A fully heard request to join can be accepted with “Go ahead.” Named cards distinguish actual playback from the selected next respondent; matching transcript labels and colors support the same identity without relying on color alone.

Morgan retains Marin. Maya uses Coral with adult, conversational delivery; the voice choice and displayed labels agree across the shared profile and hosted runtime. This is a casting revision requested after listening, not evidence of clinical authenticity or a rule about gender and voice. The two gaze observations reuse the authored encounter profile. The seating diagram is explicitly illustrative and does not establish family closeness, conflict, readiness, or any response to the learner. Observations remain separate from dialogue and are never scored or promoted into heard speech.

Both actors receive the canonical public identities in their trusted instructions: Morgan is Maya's parent and uses they/them; Maya is Morgan's adult daughter and uses she/her. These identities must remain consistent through speaker changes and alternatives. A staging audition exposed the earlier omission of pronouns from the actor projection despite correct screen labels. Prompt-contract tests establish that the provider receives the identities; live generated replies still require review and do not become reliable merely because the projection tests pass.

**Optional headphone positioning.** “Place voices in the room” keeps Morgan slightly left and Maya slightly right from the clinician's chair. It defaults off, leaving ordinary centered playback available. The browser applies a modest equal-power stereo pan (`-0.25` / `+0.25`) to existing recordings, selected from the actual clip's speaker, including a request to join and an alternative. It sends no new provider request and changes neither words nor playback speed. Switching on in the room applies when the next voice starts; switching off centers the current routed voice.
