...
test('both builders emit governance inventories matching their final tools', () => {
  ...
  try {
    ...
    ]) assert.match(residentIndex, new RegExp(expected.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
    for (const stale of ['MS3 Psychiatry Clerkship', 'MS3 Clerkship']) {
      assert.doesNotMatch(residentIndex, new RegExp(stale));
    }
  } finally {
    fs.rmSync(temporary, { recursive: true, force: true });
