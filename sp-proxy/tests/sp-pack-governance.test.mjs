...
test('managed voice independently fails closed on every review, privacy, hash, and runtime pin', () => {
  const checks = [
    ...
    ['runtime has legacy global voice', ({ runtime }) => { runtime.voiceId = 'alloy'; }],
    ['active candidate has legacy global voice', ({ pack }) => { pack.speechEngine.candidateStacks[0].voiceId = 'alloy'; refreshGovernanceHashes(pack); }],
    ['active synthesis leg has legacy global voice', ({ pack }) => { pack.speechEngine.candidateStacks[0].synthesis.voiceId = 'alloy'; refreshGovernanceHashes(pack); }],
    ...
    ['privacy URL is not HTTPS', ({ pack }) => { pack.speechEngine.privacyReview.policyUrls[0] = 'javascript:alert(1)'; refreshGovernanceHashes(pack); }],
